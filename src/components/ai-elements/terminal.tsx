...
export const TerminalCopyButton = ({
  ...
  return (
    <Button
      className={cn(
        "size-7 shrink-0 text-muted-foreground hover:bg-zinc-800 hover:text-zinc-100",
        className
      )}
    ...
    >
      {children ?? (
        <pre className="whitespace-pre-wrap break-words">
          <Ansi>{output}</Ansi>
          {isStreaming && (
            <span className="ml-0.5 inline-block h-4 w-2 animate-pulse bg-muted" />
          )}
        </pre>
      )}
    </div>
...
export const Terminal = ({
  ...
  return (
    <TerminalContext.Provider value={contextValue}>
      <div
        className={cn(
          "flex flex-col overflow-hidden rounded-lg border bg-background text-zinc-100",
          className
        )}
        {...props}
      >
